                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                     ...
                     <div class="ibox-content">
                       <!--面板开始-->   
<form class="form-horizontal" action="admin/data/editsave" method="post">
                   <input type="hidden" name="channelId" value="${channel.id}"/>
                   <input type="hidden" name="id" value="${data.id}"/>
                   <input type="hidden" name="tName" value="${data.tName}"/>
...
<c:choose>
    <c:when test="${mis.dataType==1||(mis.isCustom==1)}">
		   <c:choose>
			     <c:when test="${mis.field=='contentTem'}">
			        ...
			        <red:tempFiles temType="content" fieldName="${mis.field}" defVal="${data.contentTem}"/>
			    </c:when>
    ...
    <c:when test="${mis.dataType==2 &&(mis.isCustom==0)}">
     ...
     <c:if test="${mis.field!='level'}">
       <input type="number" name="${mis.field}"  class="form-control" value="<%=result%>"   placeholder="${mis.fieldDis}">
       </c:if>
    </c:when>
